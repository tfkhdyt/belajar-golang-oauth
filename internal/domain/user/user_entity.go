...
func (u *User) GetGitHubUserInfo(token *oauth2.Token) error {
	...
	if errNewRequest != nil {
		return fiber.NewError(fiber.StatusUnauthorized, "Failed to setup new request")
	}
	req.Header.Set("Authorization", "Bearer "+token.AccessToken)

	response, errGet := httpClient.Do(req)
	if errGet != nil {
		return fiber.NewError(fiber.StatusUnauthorized, "Failed to get github user info")
	}

	responseData, errRead := io.ReadAll(response.Body)
	if errRead != nil {
		return fiber.NewError(fiber.StatusUnauthorized, "Failed to read response body")
	}

	var user User
	if err := json.Unmarshal(responseData, &user); err != nil {
		return fiber.NewError(fiber.StatusUnauthorized, "Failed to unmarshal response")
	}
